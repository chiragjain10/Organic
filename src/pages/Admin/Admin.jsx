  ...
  const renderHeader = () => (
    <header className="flex items-center justify-between mb-8">
      <div>
        ...
        <p className="mt-1 text-sm font-medium tracking-wide text-[#1E3D2B] uppercase">
          {activeItem}
        </p>
      </div>
      ...
      <div className="px-5 py-4 flex items-center justify-between border-b border-slate-100">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-[#1E3D2B] text-white text-xs font-medium shadow-sm hover:bg-[#2F6F4E]"
          >
            Add Product
          </button>
          <span className="px-3 py-1 rounded-full bg-[#1E3D2B]/5 text-xs font-medium text-[#1E3D2B]">
          ...
          <tbody className="divide-y divide-slate-100">
            {products.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-5 py-3 text-slate-900">
                  ₹{Number(row.mrp || row.original_price || 0).toFixed(2)}
                </td>
                <td className="px-5 py-3">
                  <span
                    className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full border text-xs font-medium ${statusBadgeClasses(
                      (row.stock > 10 && "In Stock") || (row.stock > 0 && "Low Stock") || row.stock_status || "Out of Stock"
                    )}`}
                  >
                    {typeof row.stock === "number" ? row.stock : (row.stock_status || "-")}
                  </span>
                </td>
                <td className="px-5 py-3 text-slate-600">
                  {row.weight || row.net_quantity || "-"}
                </td>
                <td className="px-5 py-3">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  isActive
                    ? "bg-[#1E3D2B]/10 text-[#1E3D2B]"
                    : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                }`}
              >
                <span>{item}</span>
                {isActive && (
                  <span className="h-1.5 w-1.5 rounded-full bg-[#1E3D2B]" />
                )}
              </button>
            );
          })}
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <input
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#1E3D2B] focus:ring-1 focus:ring-[#1E3D2B] outline-none transition-all text-sm"
            placeholder="e.g. Golden Turmeric Powder"
        ...
        <div className="space-y-1">
          ...
          <select
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#1E3D2B] focus:ring-1 focus:ring-[#1E3D2B] outline-none transition-all text-sm appearance-none bg-white"
      ...
      <div className="space-y-1">
        ...
        <textarea
          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#1E3D2B] focus:ring-1 focus:ring-[#1E3D2B] outline-none transition-all text-sm min-h-[100px]"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="space-y-1">
          <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">MRP (₹)</label>
          <input
            type="number"
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#1E3D2B] focus:ring-1 focus:ring-[#1E3D2B] outline-none transition-all text-sm"
            placeholder="0.00"
        ...
        <div className="space-y-1">
          <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Weight</label>
          <input
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-1">
          <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Ingredients (optional)</label>
          <input
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#1E3D2B] focus:ring-1 focus:ring-[#1E3D2B] outline-none transition-all text-sm"
            placeholder="e.g. Turmeric, Ginger, Black Pepper"
            {...register("ingredients")}
          />
        </div>
        <div className="space-y-1">
          <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">How to Use (optional)</label>
          <input
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#1E3D2B] focus:ring-1 focus:ring-[#1E3D2B] outline-none transition-all text-sm"
            placeholder="e.g. Mix 1 tsp with warm water"
            {...register("how_to_use")}
          />
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-8 py-2.5 rounded-xl bg-[#1E3D2B] text-white text-sm font-bold shadow-lg shadow-[#1E3D2B]/20 hover:bg-[#2F6F4E] hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:translate-y-0"
        >
          {loading ? (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-8 py-2.5 rounded-xl bg-[#1E3D2B] text-white text-sm font-bold shadow-lg shadow-[#1E3D2B]/20 hover:bg-[#2F6F4E] transition-all disabled:opacity-60"
        >
